...
export const updateOrgApi = async (
  ...
  try {
    const token = localStorage.getItem("access_token");
    const response = await axios.patch<CreateOrgResponse>(
      `${BASE_URL}/portal/onboard/v1/api/gfPortal/service/organization/update`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    const errorResult = handleApiError(error, "UpdateOrgPayload");
    return {
      success: false,
      error: errorResult.error,
    };
