...
param(
    ...
    [string]$ApplicationDirectory = "C:\momas\gridflex-admin-portal",
    [string]$ReleaseArchive = "$env:USERPROFILE\gridflex-admin-release.zip",
    [string]$ReleaseChecksumFile = "$env:USERPROFILE\gridflex-admin-release.zip.sha256",
    ...
    [string]$ServiceWrapperSource = "$env:USERPROFILE\GridflexAlfuttaimAdminPortal.exe",
    [string]$ServiceWrapperChecksumFile = "$env:USERPROFILE\GridflexAlfuttaimAdminPortal.exe.sha256",
    [string]$ServiceName = "GridflexAlfuttaimAdminPortal",
    [int]$ApplicationPort = 3001,
...
function Stop-AdminPortalService {
...
function Write-DeploymentDiagnostics {
    Write-Host "--- Gridflex admin portal deployment diagnostics ---"
    $service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if ($service) {
...
function Wait-ForAdminPortal {
    ...
    do {
        ...
        $service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
        if (-not $service -or $service.Status -eq [ServiceProcess.ServiceControllerStatus]::Stopped) {
            throw "Admin portal service stopped before it became healthy. Check logs in $serviceLogDirectory."
        }
        try {
            $response = Invoke-WebRequest -Uri $healthUrl -UseBasicParsing -TimeoutSec 5
            if ($response.StatusCode -ge 200 -and $response.StatusCode -lt 400) {
                Write-Host "Admin portal health check passed at $healthUrl"
                return
            }
        }
        catch {
            Write-Host "Waiting for admin portal at $healthUrl"
        }
    } while ((Get-Date) -lt $deadline)

    $service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    $serviceState = if ($service) { $service.Status } else { "Missing" }
    throw "Admin portal did not become healthy within 90 seconds. Service state: $serviceState."
}

try {
    if (-not $applicationParent.Equals($applicationRoot, [StringComparison]::OrdinalIgnoreCase)) {
        throw "The admin portal must be deployed to a dedicated child directory of $applicationRoot. Received: $applicationDirectory"
    }
    ...
    if (-not $windowsPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw "The SSH deployment account must run with Administrator rights to manage the admin portal service."
    }
...
<service>
  <id>$ServiceName</id>
  <name>Gridflex Alfuttaim Admin Portal</name>
  <description>Gridflex Alfuttaim Next.js admin portal</description>
...
"@
    ...
    if ($serviceTouched) {
        try {
            ...
            [Console]::Error.WriteLine("Could not stop the failed admin portal service: $($_.Exception.Message)")
        }
    }
    ...
    if (-not $serviceExisted -and ($serviceInstalled -or $newServiceExists)) {
        try {
            ...
            [Console]::Error.WriteLine("Could not uninstall the failed admin portal service: $($_.Exception.Message)")
        }
    }
    ...
    if ($serviceExisted -and $serviceWasRunning -and (Test-Path -LiteralPath $applicationDirectory)) {
        try {
            Start-Service -Name $ServiceName
            (Get-Service -Name $ServiceName).WaitForStatus([ServiceProcess.ServiceControllerStatus]::Running, [TimeSpan]::FromSeconds(45))
            Write-Host "Previous admin portal release was restored and restarted."
        }
        catch {
            [Console]::Error.WriteLine("Could not restart the previous admin portal service: $($_.Exception.Message)")
        }
    }
