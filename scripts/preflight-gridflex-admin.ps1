...
function Assert-DirectoryWritable {
    ...
    $probeDirectory = Join-Path $Directory ".gridflex-admin-preflight-$([Guid]::NewGuid().ToString('N'))"
    try {
        New-Item -ItemType Directory -Path $probeDirectory | Out-Null
        [IO.File]::WriteAllText(
            (Join-Path $probeDirectory "write-test.txt"),
            "Gridflex admin portal deployment write test",
...
if ($service) {
    ...
    Write-Host "Existing admin portal service: $ServiceName ($($service.Status))"
}
...
Write-Host "Admin portal directory: $applicationDirectory"
Write-Host "Free space: $([Math]::Round($drive.Free / 1GB, 2)) GB"
Write-Host "Service name: $ServiceName"
Write-Host "Admin portal port: $ApplicationPort"
